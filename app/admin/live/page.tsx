...
// (les mêmes pages vues / clics que /admin/usage, mais sur les dernières
...
export default async function AdminLivePage({
  ...
  const rows = [...byUser.values()]
    .map((u) => {
      ...
      return { ...u, events: events.slice(0, 8), lastAt, sinceMs, online: sinceMs <= ONLINE_WINDOW_MS };
    })
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-8 text-chalk">
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <Link href={`/admin/usage?secret=${secret}`} className="text-xs font-semibold text-glow hover:underline">
          Voir l&apos;usage global →
        </Link>
      </div>
      <div className="mb-1 flex items-center gap-2">
        ...
        <h1 className="font-condensed text-3xl font-bold uppercase">En direct</h1>
      </div>
