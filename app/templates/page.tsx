      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', zIndex: 1 }}>
        {displayTemplates.map((t, i) => (
          <motion.div
            ...
            whileTap={{ scale: 0.98 }}
          >
            <Link href={`/create?template=${t.slug}`} style={{ textDecoration: 'none', display: 'block' }}>
              <div style={{
                padding: '20px 22px', borderRadius: '22px',
                background: `linear-gradient(135deg, ${t.color_from}18, ${t.color_to}12)`,
                border: `1px solid ${t.color_from}28`,
                display: 'flex', alignItems: 'center', gap: '16px',
                position: 'relative', overflow: 'hidden',
              }}>
                {t.is_premium && (
                  <div style={{ position: 'absolute', top: '10px', right: '12px', background: 'linear-gradient(135deg, #f59e0b, #a855f7)', borderRadius: '7px', padding: '2px 8px', fontSize: '10px', fontWeight: 800, color: '#fff' }}>Inside+</div>
                )}
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 800, fontSize: '1.05rem', color: '#f0f0f5', display: 'flex', alignItems: 'center', gap: '6px' }}>
                    {t.name}{t.is_premium && <span style={{ fontSize: '.85rem' }}>🔒</span>}
                  </div>
                ...
                <span style={{ color: 'rgba(240,240,245,0.30)', fontSize: '1.1rem', flexShrink: 0 }}>›</span>
              </div>
            </Link>
          </motion.div>
        ))}
